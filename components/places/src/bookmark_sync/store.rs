...
mod tests {
    ...
    type ExpectedSyncedItem<'a> = (
        &'static str,
        &'a SyncedBookmarkItem,
        Option<Box<dyn Fn(&mut SyncedBookmarkItem) -> &mut SyncedBookmarkItem>>,
    );

    fn check_expected_synced_items(conn: &PlacesDb, table: &[ExpectedSyncedItem<'_>]) {
        for (guid, base, func) in table {
            let actual = SyncedBookmarkItem::get(conn, &SyncGuid::from(guid))
                .expect("Should fetch synced bookmark item")
                .expect("Expected synced item should exist");
    ...
    fn test_apply_complex_bookmark_tags() -> Result<()> {
        ...
        let expected_synced_items: &[ExpectedSyncedItem<'_>] = &[
            (
                "bookmarkAAA1",
                &synced_item_for_a,
                Some(Box::new(|a| {
        ...
        check_expected_synced_items(&writer, expected_synced_items);
